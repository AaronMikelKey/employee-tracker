...
const addEmployee = async () => {
	...
	inquirer
		...
		.then((data) => {
			...
			let query
			if (data[4] === 'NULL') {
				//set query to work if there is no manager
				query = `INSERT INTO employees (first_name, last_name, role_id, department_id) VALUES (?,?,?,?)`
			} else {
				//add manager id to query
				query = `INSERT INTO employees (first_name, last_name, role_id, department_id, manager_id) VALUES (?,?,?,?,?)`
			}
